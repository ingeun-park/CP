...
public class MiniCPrintListener extends MiniCBaseListener {
   ...
   public void exitVar_decl(MiniCParser.Var_declContext ctx) {
      ...
      if (ctx.getChildCount() == 6 && ctx.getChild(3).getText().equals("["))// array일경우
         str = nextTexts.get(ctx.type_spec()) + " " + ctx.IDENT().getText() + ctx.getChild(3).getText()
         ...
         str = nextTexts.get(ctx.type_spec()) + " " + ctx.IDENT().getText() + " " + ctx.getChild(2).getText() + " "
   ...
   public void exitProgram(MiniCParser.ProgramContext ctx) {
      ...
      while (ctx.getChildCount() > index && ctx.getChild(index) == ctx.decl(index)) { // 더 있을경우 반복
         s1 = nextTexts.get(ctx.decl(index++)) + "\n";
         s2 = s2 + s1;
      }
   ...
   public void exitFun_decl(MiniCParser.Fun_declContext ctx) {
      nextTexts.put(ctx, nextTexts.get(ctx.type_spec()) + " " + ctx.IDENT().getText() + "("
   ...
   public void exitParam(MiniCParser.ParamContext ctx) {
      String p = nextTexts.get(ctx.type_spec()) + " " + ctx.IDENT().getText();
      nextTexts.put(ctx, p);
   }
